...
pub fn draw_metadata_edit_popup(f: &mut Frame, app: &mut App) {
    ...
    for (i, label) in FIELD_LABELS.iter().enumerate() {
        ...
        draw_metadata_field(f, form, &current_field, label, rows[i], editing);
    }
...
fn draw_metadata_field(
    ...
    if *current_field == Field::Description {
        ...
        f.render_widget(&form.description, area);
        return;
    }
    ...
    if editing && is_selected {
        let x = match current_field {
            ...
            Field::Description => unreachable!(
                "Description field is not a Input, but a TextArea, and should be already processed"
            ),
